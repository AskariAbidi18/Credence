...
export async function renderDocuments() {
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">
          Documents processed by Credence
        </p>
      </div>
  ...
  try {
    ...
    renderDocumentList(content.querySelector("#documents-content"), documents);
  } catch (error) {
    console.error("Document history load failed:", error);

    const target = content.querySelector("#documents-content");
...
function renderDocumentList(
  ...
  if (statusFilter)
    docs = docs.filter((d) => d.extraction_status === statusFilter);
    ...
    ${
      ...
      <div class="filter-bar">
        <div
          ...
          style="flex:1; max-width:22rem; position:relative;"
        >
          ...
          <input
            ...
            class="form-input"
            id="search-input"
            placeholder="Search filename, ID, type..."
        ...
        <select class="filter-select" id="type-filter">
          ...
          <option value="payslip" ${typeFilter === "payslip" ? "selected" : ""}>
            Payslip
          </option>
          <option value="bank_statement" ${
            typeFilter === "bank_statement" ? "selected" : ""
          }>
            Bank Statement
          </option>
          <option value="tax_return" ${
            typeFilter === "tax_return" ? "selected" : ""
          }>
            Tax Return
          </option>
          <option value="kyc" ${typeFilter === "kyc" ? "selected" : ""}>
            KYC
          </option>
        </select>
        ...
        <select class="filter-select" id="status-filter">
          ...
          <option value="partial" ${
            statusFilter === "partial" ? "selected" : ""
          }>
            Partial
          </option>
          <option value="failed" ${statusFilter === "failed" ? "selected" : ""}>
            Failed
          </option>
        </select>
...
function renderTable(docs) {
  const rows = docs
    .map((d) => {
      ...
      return `
      <tr style="cursor:pointer;" onclick="window.location.hash='/document/${escHtml(d.document_id)}'">
        <td>
        ...
        <td class="text-muted text-xs font-mono">${escHtml(d.document_id.slice(0, 12))}...</td>
